...
describe('validatePayload', () => {
	...
	it('should throw error for empty environments', () => {
		...
		expect(() => validatePayload(payload)).toThrow('Invalid environments: must be a non-empty array');
	});
	...
	it('should throw error for invalid issued_at date', () => {
		...
		expect(() => validatePayload(payload)).toThrow('Invalid issued_at: must be a valid ISO 8601 date');
	});

	it('should throw error for invalid expires_at date', () => {
		const payload = { ...validPayload, expires_at: 'invalid-date' };
		expect(() => validatePayload(payload)).toThrow('Invalid expires_at: must be a valid ISO 8601 date');
	});
	...
	it('should throw error when expires_at is before issued_at', () => {
		...
		expect(() => validatePayload(payload)).toThrow('Invalid dates: expires_at must be after issued_at');
	});
	...
	it('should throw error for negative max_runs_per_day', () => {
		...
		expect(() => validatePayload(payload)).toThrow('Invalid max_runs_per_day: must be a positive number');
	});
